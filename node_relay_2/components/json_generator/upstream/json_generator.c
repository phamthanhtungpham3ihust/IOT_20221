...
void json_gen_temp_humidity(json_gen_test_result_t *result, char *key_temp, float value_temp, 
							char *key_humi, float value_humi, char *arr)
{
	char buf[20];
	...
	json_gen_obj_set_float(&jstr, key_temp, value_temp);
	json_gen_obj_set_float(&jstr, key_humi, value_humi);
	json_gen_end_object(&jstr);
	json_gen_str_end(&jstr);
	strcpy(arr, result->buf);
}

void json_gen_string_temp_humidity(json_gen_test_result_t *result, char *key_temp, char *value_temp, 
							char *key_humi, char *value_humi, char *arr)
{
	char buf[20];
    memset(result, 0, sizeof(json_gen_test_result_t));
	json_gen_str_t jstr;
	json_gen_str_start(&jstr, buf, sizeof(buf), flush_str, result);
	json_gen_start_object(&jstr);
	json_gen_obj_set_string(&jstr, key_temp, value_temp);
	json_gen_obj_set_string(&jstr, key_humi, value_humi);
	json_gen_end_object(&jstr);
	json_gen_str_end(&jstr);
	strcpy(arr, result->buf);
}
void json_gen_float_button(json_gen_test_result_t *result, char *key_button, float value_button, char *arr)
{
	char buf[20];
    memset(result, 0, sizeof(json_gen_test_result_t));
	json_gen_str_t jstr;
	json_gen_str_start(&jstr, buf, sizeof(buf), flush_str, result);
	json_gen_start_object(&jstr);
	json_gen_obj_set_float(&jstr, key_button, value_button);
	json_gen_end_object(&jstr);
	json_gen_str_end(&jstr);
	strcpy(arr, result->buf);
}

void json_gen_data(json_gen_test_result_t *result, char *key_temp, char *value_temp, 
							char *key_humi, char *value_humi, char *key_button, float value_button ,char *arr)
{
	char buf[20];
    memset(result, 0, sizeof(json_gen_test_result_t));
	json_gen_str_t jstr;

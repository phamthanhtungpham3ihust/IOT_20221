...
static const char *TAG = "NODE_RELAY_2";
char* check_button2_on;

void mqtt_data_callback(char *data, uint16_t len)
{
    check_button2_on = strstr(data, "ON2");

    // When receiving data "ON2" from topic "button2", relay control signal is HIGH
    if(check_button2_on != NULL){
        esp_output_set_level(23, 1);
        //esp_output_set_level(2, 1);
    }
    // When not receiving data "ON2" from topic "button2", relay control signal is LOW
    else{
...
void app_main(void)
{
    ...
    // Initialize relay control pin at pin 23
